...
async def show_admin_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    admin_menu_text = "اختر الخدمة التالية:"
...
"""
        ...
        await update.message.reply_text(
            ...
            "💡 يمكنك تجاهل هذا الخطأ والعودة لإدارة المستخدمين",
            reply_markup=reply_markup
        )
        return ADD_USER_DATA
    except Exception as e:
...
async def system_settings_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "delete_old_invoices":
        return await show_delete_old_invoices_confirmation(update, context)
    elif query.data == "reset_statistics":
        return await show_reset_statistics_confirmation(update, context)
    elif query.data == "reset_system":
        return await show_reset_system_confirmation(update, context)
    elif query.data == "back_to_admin":
        ...
        return ADMIN_MENU
    elif query.data == "confirm_delete_old_invoices":
        return await delete_old_invoices_handler(update, context)
    elif query.data == "confirm_reset_statistics":
        return await reset_statistics_handler(update, context)
    elif query.data == "confirm_reset_system":
        return await reset_system_handler(update, context)
    elif query.data == "cancel_operation":
        return await show_system_settings(update, context)

async def show_delete_old_invoices_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """عرض تأكيد حذف الفواتير القديمة"""
...
"""
    ...
    keyboard = [
        [InlineKeyboardButton("✅ نعم، احذف الفواتير", callback_data="confirm_delete_old_invoices")],
...
async def show_reset_statistics_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """عرض تأكيد تصفير الإحصائيات"""
    # الحصول على إحصائيات النظام
    stats = db_manager.get_system_stats()
...
"""
    ...
    keyboard = [
        [InlineKeyboardButton("✅ نعم، صفر الإحصائيات", callback_data="confirm_reset_statistics")],
        [InlineKeyboardButton("❌ إلغاء العملية", callback_data="cancel_operation")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def show_reset_system_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """عرض تأكيد إعادة تعيين النظام"""
    # الحصول على إحصائيات النظام
    stats = db_manager.get_system_stats()
...
✅ سيتم الاحتفاظ بـ:
• المستخدمين: {stats['users_count']} مستخدم
...
"""
    ...
    keyboard = [
        [InlineKeyboardButton("🚨 نعم، أعيد تعيين النظام", callback_data="confirm_reset_system")],
        [InlineKeyboardButton("❌ إلغاء العملية", callback_data="cancel_operation")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def delete_old_invoices_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """معالج حذف الفواتير القديمة بعد التأكيد"""
    try:
...
async def reset_statistics_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """معالج تصفير الإحصائيات بعد التأكيد"""
    try:
        # إرسال رسالة تأكيد
...
async def reset_system_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """معالج إعادة تعيين النظام بعد التأكيد"""
    try:
        # الحصول على إحصائيات النظام قبل الحذف
        stats_before = db_manager.get_system_stats()
    ...
    try:
        ...
        conv_handler = ConversationHandler(
            ...
            states={
                ...
                ADD_USER_ROLE: [
                    MessageHandler(filters.TEXT & ~filters.COMMAND, user_management_handler),
                    CallbackQueryHandler(add_user_role_handler, pattern="^(role_|back_to_user_management)$")
                ],
                ADD_USER_DATA: [
                    MessageHandler(filters.TEXT & ~filters.COMMAND, user_management_handler),
                    CallbackQueryHandler(user_management_callback_handler, pattern="^(back_to_admin|back_to_user_management)$")
                ],
                PASSWORD_MANAGEMENT: [
